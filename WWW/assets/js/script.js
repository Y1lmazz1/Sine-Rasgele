   ...
   async function setupDynamicHero() {
    try {
        const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${CONFIG.API_KEY}&language=tr-TR&sort_by=popularity.desc&vote_average.gte=8.0&vote_average.lte=10&vote_count.gte=1000&page=1`);
        
        if (!res.ok) throw new Error(`API hatası: ${res.status}`);
        
        const data = await res.json();
        if (!data.results) throw new Error("API'den film gelmedi");

        // 1. Veriyi global değişkene ata (Dışarıda tanımladığın heroMovies dizisi)
        heroMovies = data.results.filter(movie => movie.vote_average >= 8);
        
        console.log("Filtrelenmiş Filmler:", heroMovies);

        // 2. VERİ GELDİKTEN SONRA renderHero'yu çalıştır
        ...
        console.error("Hero Slider Hatası:", error);
    }
}
    ...
    if (!movie) {
        alert("Lütfen önce bir film seçin!");
        return;
    }
    ...
    shareBtn.innerHTML = "⌛ HAZIRLANIYOR...";
    shareBtn.disabled = true;

    try {
        // İçeriği temizle ve doldur
        cardTitle.innerText = movie.title?.toUpperCase() || "BİLİNMEYEN FİLM";
        cardStars.innerHTML = movie.vote_average ? "⭐".repeat(Math.round(movie.vote_average / 2)) : "";

        const posterPath = movie.poster_path 
            ? `https://images.weserv.nl/?url=https://image.tmdb.org/t/p/w500${movie.poster_path}&w=500&t=${new Date().getTime()}`
            : "https://via.placeholder.com/500x750?text=Afis+Yok";

        // Template'i geçici olarak görünür yap (ekran dışında)
        ...
        await new Promise((resolve, reject) => {
            ...
            cardPoster.onerror = () => reject(new Error("Afiş yüklenemedi."));
            cardPoster.src = posterPath;
        });

        // Render için minik bir bekleme
        ...
        link.download = `${movie.title?.replace(/\s+/g, '_') || 'sine_rasgele'}.png`;
        link.href = image;
        link.click();
